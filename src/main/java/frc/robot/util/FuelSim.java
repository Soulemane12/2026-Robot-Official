...
public class FuelSim {
    ...
    public FuelSim(String tableKey) {
        ...
        trajectoryPublisher = NetworkTableInstance.getDefault()
                .getStructArrayTopic(tableKey + "/Trajectories", Translation3d.struct)
                .publish();
    }
    ...
    protected StructArrayPublisher<Translation3d> trajectoryPublisher;
    ...
    public void logFuels() {
        ...
        // Calculate and publish trajectories for all in-flight fuels
        ArrayList<Translation3d> trajectoryPoints = new ArrayList<>();
        for (Fuel fuel : fuels) {
            // Only show trajectory for balls that are airborne with significant upward/horizontal velocity
            // Hide trajectory when ball is on ground or rolling slowly
            boolean isAirborne = fuel.pos.getZ() > FUEL_RADIUS + 0.1;
            boolean hasSignificantVelocity = fuel.vel.getNorm() > 1.5;
            boolean hasVerticalVelocity = Math.abs(fuel.vel.getZ()) > 0.3;

            if (isAirborne && hasSignificantVelocity && hasVerticalVelocity) {
                calculateTrajectory(fuel, trajectoryPoints);
            }
        }
        trajectoryPublisher.set(trajectoryPoints.toArray(Translation3d[]::new));
    }

    /**
     * Calculates predicted trajectory points for a fuel ball
     * @param fuel The fuel to calculate trajectory for
     * @param trajectoryPoints List to add trajectory points to
     */
    private void calculateTrajectory(Fuel fuel, ArrayList<Translation3d> trajectoryPoints) {
        Translation3d pos = fuel.pos;
        Translation3d vel = fuel.vel;

        // Predict up to 2 seconds of flight time with 0.1s steps
        int steps = 20;
        ...
        for (int i = 0; i < steps; i++) {
            ...
            // Simple ballistic trajectory (gravity only, no air resistance for prediction)
            pos = pos.plus(vel.times(dt));
            vel = vel.plus(GRAVITY.times(dt));

            // Stop predicting if ball would hit ground
            if (pos.getZ() <= FUEL_RADIUS) {
                break;
            }

            // Stop if out of field bounds
            if (pos.getX() < 0 || pos.getX() > FIELD_LENGTH ||
                pos.getY() < 0 || pos.getY() > FIELD_WIDTH) {
                break;
            }
        }
    }
